...
pub fn init() {
    let env_filter = tracing_subscriber::EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        tracing_subscriber::EnvFilter::new("deck_service=info,tower_http=info,warn")
    });
...
fn component_name(target: &str) -> &str {
    ...
    match parts.next() {
        Some("deck_service") => parts.next().unwrap_or("main"),
...
fn component_color(component: &str) -> &'static str {
    match component {
        ...
        "handlers" | "http" => COLOR_GREEN,
        "bridge" | "ffi" => COLOR_MAGENTA,
        "masterdata" | "models" => COLOR_BRIGHT_MAGENTA,
        "state" => COLOR_CYAN,
        "error" => COLOR_RED,
        "tower_http" => COLOR_BLUE,
        "engine" | "pool" => COLOR_BRIGHT_BLUE,
        _ => COLOR_BRIGHT_CYAN,
    }
}
